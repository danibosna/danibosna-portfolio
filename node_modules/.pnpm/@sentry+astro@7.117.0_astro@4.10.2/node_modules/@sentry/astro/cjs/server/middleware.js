...
const handleRequest = options => {
  ...
  return async (ctx, next) => {
    ...
    if (node.getActiveSpan()) {
      return instrumentRequest(ctx, next, handlerOptions);
    }
    return node.runWithAsyncContext(() => {
      return instrumentRequest(ctx, next, handlerOptions);
    });
  };
};
...
async function instrumentRequest(
  ...
  utils.addNonEnumerableProperty(locals, '__sentry_wrapped__', true);

  const { method, headers } = ctx.request;

  return node.continueTrace(
    {
    ...
    async () => {
      ...
      if (options.trackClientIp) {
        node.getCurrentScope().setUser({ ip_address: ctx.clientAddress });
      }
      ...
      try {
        ...
        const res = await node.startSpan(
          {
            attributes: {
              [core.SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN]: 'auto.http.astro',
            },
            ...
            data: {
              method,
              url: utils.stripUrlQueryAndFragment(ctx.url.href),
          ...
          async span => {
            ...
            if (span && originalResponse.status) {
              core.setHttpStatus(span, originalResponse.status);
            }
...
function addMetaTagToHead(htmlChunk, scope, client, span) {
  ...
  const { sentryTrace, baggage } = meta.getTracingMetaTags(span, scope, client);
  const content = `<head>\n${sentryTrace}\n${baggage}\n`;
...
exports.interpolateRouteFromUrlAndParams = interpolateRouteFromUrlAndParams;
